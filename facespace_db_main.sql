...
CREATE TRIGGER dropUserTrigger
	BEFORE DELETE
		ON USERS
		FOR EACH ROW
	...
	BEGIN
		DELETE FROM GROUP_MEMBERS
			WHERE (GROUP_MEMBERS.USER_ID = :old.USER_ID);

		UPDATE MESSAGES
			SET SENDER_ID = null
			WHERE (MESSAGES.SENDER_ID = :old.USER_ID);

		UPDATE MESSAGES
			SET RECEIVER_ID = null
			WHERE (MESSAGES.RECEIVER_ID = :old.USER_ID);

		DELETE FROM MESSAGES
			WHERE (MESSAGES.SENDER_ID = null
				AND MESSAGES.RECEIVER_ID = null);
